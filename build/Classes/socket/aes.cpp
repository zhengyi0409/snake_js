...
int aes_setks_encrypt(const uint8_t *keybuf, int kbits, aes_ks_t *ks) {
	...
	if (!keybuf || !ks)
		return -1;
	if (kbits != 128 && kbits != 192 && kbits != 256)
		return -2;
	...
	rk[0] = GETU32(keybuf	  );
	rk[1] = GETU32(keybuf +	 4);
	rk[2] = GETU32(keybuf +	 8);
	rk[3] = GETU32(keybuf + 12);
	if (kbits == 128) {
		while (1) {
			temp  = rk[3];
			rk[4] = rk[0] ^
	...
	rk[4] = GETU32(keybuf + 16);
	rk[5] = GETU32(keybuf + 20);
	if (kbits == 192) {
		while (1) {
			temp = rk[ 5];
			rk[ 6] = rk[ 0] ^
	...
	rk[6] = GETU32(keybuf + 24);
	rk[7] = GETU32(keybuf + 28);
	if (kbits == 256) {
		while (1) {
			temp = rk[ 7];
			rk[ 8] = rk[ 0] ^
...
int aes_setks_decrypt(const uint8_t *keybuf, int kbits, aes_ks_t *ks)
{
	...
	status = aes_setks_encrypt(keybuf, kbits, ks);
	if (status < 0)
		return status;
	...
	for (i = 0, j = 4*(ks->rounds); i < j; i += 4, j -= 4) {
...
{
	const uint8_t *vec = iv;

	while (blks--) {
		AES_BLOCK_OP(out, in,	=);
		AES_BLOCK_OP(out, vec, ^=);
		aes_ecb_encrypt(out, out, ks);
		vec  = out;
		in  += AES_BLOCK_SIZE;
		out += AES_BLOCK_SIZE;
	}
